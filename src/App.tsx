...
function App() {
  ...
  const copyToClipboard = () => {
    if (refPasswordGenerated.current) {
      navigator.clipboard.writeText(refPasswordGenerated.current.value).catch((e) => {
        console.error(e)
      })
    }
  }
        ...
        <main className='min-h-[78vh]'>
          ...
          <div>
            <div className='flex w-full mt-4 gap-[1px]'>
              <Input refInput={refPasswordGenerated} props={{ type: 'text', style: { width: '100%' } }} />
              <button onClick={copyToClipboard} className='bg-secondary shadow-lg border border-primary py-[0.3em] px-[0.6em] hover:shadow-lg hover:-translate-y-1 active:translate-y-0 duration-300 text-primary'>
